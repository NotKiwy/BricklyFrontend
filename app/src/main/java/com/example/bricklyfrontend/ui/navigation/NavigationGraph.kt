package com.example.bricklyfrontend.ui.navigation
...
fun BricklyNavigation(
    ...
    fun navToTab(route: String) {
        val tabRoot = if (UserPreferences.isLoggedIn(context)) "meetings" else startDestination
    ...
    NavHost(
        ...
        composable("create_meeting") {
            ...
            CreateMeetingScreen(
                onBack = { navController.popBackStack() },
                onMeetingCreated = {
                    Toast.makeText(ctx, "Сходка создана", Toast.LENGTH_SHORT).show()
                    navController.navigate("meetings") {
